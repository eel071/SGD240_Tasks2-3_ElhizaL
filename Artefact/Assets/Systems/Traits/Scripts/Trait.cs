...
public class TraitElement
{
    ...
    public float Apply(AIStat targetStat, Trait.ETargetType targetType, float currentValue)
    {
        if(targetStat == LinkedStat)
        {
            ...
            {
                if (currentValue > 0f) { currentValue *= Impact_PositiveScale; }
                else if (currentValue < 0f) { currentValue *= Impact_NegativeScale; }
            }
            
            else
            {
                if (currentValue > 0f) { currentValue *= Scoring_PositiveScale; }
                else if (currentValue < 0f) { currentValue *= Scoring_NegativeScale; }
            }
        }
